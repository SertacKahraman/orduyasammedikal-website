...
const blogMenuItems = [
  {
    label: "Hasta Bakımı",
    post: blogPosts.find((post) => post.slug === "evde-hasta-bakimi-en-uygun-hasta-yatagi-nasil-secilir"),
  },
  {
    label: "Tansiyon Rehberi",
    post: blogPosts.find((post) => post.slug === "tansiyon-aleti-alirken-dikkat-edilmesi-gerekenler"),
  },
  {
    label: "Günlük Sağlık",
    post: blogPosts.find((post) => post.slug === "ortopedik-tabanlik-ayak-ve-bel-agrilari"),
  },
  {
    label: "Anne & Bebek",
    post: blogPosts.find((post) => post.slug === "ates-olcer-secimi-temassiz-mi-kulaktan-mi"),
  },
      ...
      <div className="invisible absolute left-1/2 top-full z-50 w-[640px] -translate-x-1/2 translate-y-3 pt-4 opacity-0 transition duration-300 ease-out group-hover:visible group-hover:translate-y-0 group-hover:opacity-100">
        ...
        >
          <div className="relative min-h-[380px] overflow-hidden rounded-[14px] bg-[#E8C9B2]">
            <Image
              src={services[0].cardImage ?? services[0].image}
              alt="Ordu Yaşam Medikal ürünleri"
              fill
              sizes="400px"
            ...
            <div className="absolute inset-x-5 bottom-5 text-white">
              <p className="font-serif text-2xl font-medium leading-tight">{businessInfo.name}</p>
              <a
    ...
    >
      ...
      >
        ...
        <Link
          ...
          aria-label="Ordu Yaşam Medikal ana sayfa"
        >
          <Image
            ...
            height={87}
            priority
            sizes={isScrolled ? "(max-width: 639px) 90px, (max-width: 1023px) 115px, 130px" : "(max-width: 639px) 130px, (max-width: 1023px) 160px, 180px"}
            className={
              isScrolled
                ? "h-auto w-[90px] object-contain transition-all duration-300 sm:w-[115px] lg:w-[130px]"
                : "h-auto w-[130px] object-contain transition-all duration-300 sm:w-[160px] lg:w-[180px]"
            }
          />
        </Link>
