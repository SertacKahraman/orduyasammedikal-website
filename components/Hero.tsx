...
export default function Hero({ title, description }: HeroProps) {
  return (
    <section className="relative isolate min-h-[100svh] overflow-hidden bg-[#E8C9B2]">
      <Image
        src="/assets/images/hero/ordu-yasam-medikal-hero-background.webp"
        alt="Ordu Yaşam Medikal tıbbi cihazlar ve sağlık ürünleri"
        fill
        priority
        sizes="100vw"
      ...
      <div className="relative z-10 mx-auto flex min-h-[100svh] max-w-7xl items-center px-4 pb-10 pt-28 sm:px-6 sm:pt-32 lg:px-8">
        <div className="grid max-w-[760px] gap-5 sm:gap-7">
          <div className="flex flex-wrap items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#7A3429] sm:gap-3 sm:text-xs sm:tracking-[0.3em]">
            ...
            <span>Medikal ürün satış ve kurulum</span>
          </div>
          ...
          <div className="flex w-fit max-w-full flex-nowrap gap-0.5 whitespace-nowrap rounded-[18px] border border-[#DDB89F]/60 bg-[#F7EFE6]/52 px-1.5 py-2 text-[clamp(10px,2.8vw,11px)] font-semibold uppercase tracking-[-0.01em] text-[#7A4A3C] backdrop-blur-sm sm:gap-2 sm:rounded-full sm:px-4 sm:text-xs sm:tracking-[0.2em]">
            <span>Tekerlekli Sandalye</span>
            <span className="text-[#7A3429]/60">·</span>
            <span>Hasta Yatağı</span>
            <span className="text-[#7A3429]/60">·</span>
            <span>Solunum Cihazı</span>
          </div>
        </div>
      </div>
    </section>
