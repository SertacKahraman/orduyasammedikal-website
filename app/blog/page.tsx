...
const blogDescription =
  "Hasta bakımı, medikal cihazlar, ortopedik ürünler ve evde sağlık takibi hakkında Ordu Yaşam Medikal tarafından hazırlanan pratik rehberler.";

export const metadata: Metadata = generateStaticPageMetadata(
  "Medikal Ürün ve Sağlık Rehberi",
  blogDescription,
  "/blog",
);
...
export default function BlogPage() {
  ...
  const featuredPost =
    blogPosts.find((post) => post.slug === "evde-hasta-bakimi-en-uygun-hasta-yatagi-nasil-secilir") ??
    blogPosts[0];
  const remainingPosts = blogPosts.filter((post) => post.id !== featuredPost?.id);
  const blogSchema = {
    "@context": "https://schema.org",
    "@type": "Blog",
    name: "Ordu Yaşam Medikal Blog",
  ...
  return (
    <>
      ...
      <PageIntro
        ...
        eyebrow="Ordu Medikal Ürün Rehberi"
        label="Blog & Haberler"
        title="Sağlık ve ürün seçimini kolaylaştıran rehberler"
        description="Hasta bakımı, medikal cihaz kullanımı, ortopedik ürünler ve evde sağlık takibi hakkında en çok merak edilen konuları sade bir yapıda topladık."
        showTopBar={false}
      />
          ...
          <div className="featured-post-intro mx-auto grid max-w-7xl overflow-hidden border border-[#DDB89F]/45 bg-white shadow-[0_24px_80px_rgba(58,42,36,0.1)] lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
            <Link href={`/blog/${featuredPost.slug}`} className="relative min-h-[320px] overflow-hidden bg-[#E8C9B2] sm:min-h-[420px]">
              <Image
                ...
                alt={`${featuredPost.title} - ${businessInfo.name} blog`}
                fill
          ...
          <div className="flex flex-col justify-between gap-5 border-b border-[#DDB89F]/45 pb-7 sm:flex-row sm:items-end">
            <div>
              ...
              <h2 className="mt-3 font-serif text-4xl font-medium text-[#3A2A24]">
                Medikal ürün ve evde bakım içerikleri
              </h2>
            </div>
          </div>
            ...
            {remainingPosts.map((post, index) => (
              ...
              >
                <Link href={`/blog/${post.slug}`} className="block overflow-hidden bg-[#E8C9B2]">
                  <Image
                    src={post.image}
                    alt={`${post.title} - ${businessInfo.name} blog`}
                    width={760}
                    height={520}
